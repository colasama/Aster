...
impl BenchmarkReport {
    pub fn regressions_against(
        ...
        .filter_map(|(metric, current, previous)| {
            let median = percent_change(current.median, previous.median);
            let p95 = percent_change(current.p95, previous.p95);
...
impl BenchmarkStatistics {
    fn from_frames(frames: &[FrameMetrics]) -> Self {
        Self {
            cpu_ms: distribution(frames.iter().map(|frame| f64::from(frame.cpu_ms))),
            gpu_ms: distribution(frames.iter().map(|frame| f64::from(frame.gpu_ms))),
            frame_ms: distribution(frames.iter().map(|frame| f64::from(frame.frame_ms))),
            frames_per_second: distribution(frames.iter().map(|frame| {
            ...
            estimated_vram_mb: distribution(
                frames
                    .iter()
                    .map(|frame| frame.vram_bytes as f64 / (1024.0 * 1024.0)),
            ),
            draw_calls: distribution(frames.iter().map(|frame| f64::from(frame.draw_calls))),
            dispatches: distribution(frames.iter().map(|frame| f64::from(frame.dispatches))),
            dirty_nodes: distribution(frames.iter().map(|frame| f64::from(frame.dirty_nodes))),
...
mod tests {
    ...
    #[test]
    fn discards_warmup_and_reports_interpolated_percentiles() {
    ...
    fn detects_median_and_tail_regressions() {
        let baseline = report_with_times(10.0, 12.0);
        let current = report_with_times(10.6, 13.3);
    ...
    fn report_with_times(median: f64, p95: f64) -> BenchmarkReport {
